...
public class BuildingGenerator : MonoBehaviour {
    ...
    public void GenerateFromPlot(PlotData plot, BuildingTypeSettings settings, int buildingType, Vector3 offset, int richness) {
        GeneratorSettings = settings.GeneratorSettings as RandomSettings;
        
        if (mesh != null)
            mesh.Clear();
        Setup(settings);
        SetupMaterials(settings, richness);

        var generatedMesh = Generate(plot, settings, buildingType, offset);
        
        var finalMesh = MeshUtils.Translate(generatedMesh, -0.5f*plot.Bounds.size.ToVec3() + new Vector3(0.5f, 0, 0.5f) + offset);
    ...
    public virtual MeshData Generate(PlotData plot, BuildingTypeSettings settings, int buildingType, Vector3 offset) {
    ...
    private void SetupMaterials(BuildingTypeSettings settings, int richness) {
        int index = Mathf.Clamp(richness, 0, settings.RichnessBasedMaterials.Count);
        var sharedMaterials = new Material[6];
        meshRenderer.sharedMaterial = settings.RichnessBasedMaterials[index].WallMaterial;
        sharedMaterials[0] = settings.RichnessBasedMaterials[index].WallMaterial;
        sharedMaterials[1] = settings.RichnessBasedMaterials[index].RoofMaterial;
        sharedMaterials[2] = settings.RichnessBasedMaterials[index].FeatureMaterial1 != null ? settings.RichnessBasedMaterials[index].FeatureMaterial1 : settings.RichnessBasedMaterials[index].WallMaterial;
        sharedMaterials[3] = settings.RichnessBasedMaterials[index].WindowMaterial;
        sharedMaterials[4] = settings.RichnessBasedMaterials[index].FeatureMaterial2 != null ? settings.RichnessBasedMaterials[index].FeatureMaterial2 : settings.RichnessBasedMaterials[index].WallMaterial;
        sharedMaterials[5] = settings.RichnessBasedMaterials[index].FeatureMaterial3 != null ? settings.RichnessBasedMaterials[index].FeatureMaterial3 : settings.RichnessBasedMaterials[index].WallMaterial;
    ...
    protected MeshData GenWalls(List<Vector2Int> path) {
