...
public abstract class MeshGenerator {
    ...
    private void ApplyTransformation() {
        for (var i = 0; i < meshData.Vertices.Count; i++) {
            meshData.Vertices[i] = rotation * (meshData.Vertices[i] * GlobalSettings.Instance.GridSize) + position * GlobalSettings.Instance.GridSize;
        }
    }
    ...
    protected void AddQuad(Vector3 v0, Vector3 v1, Vector3 v2, Vector3 v3, int submesh, bool flip = false, UVSettings uvSettings = Default) {
        ...
        var uvs = UVUtils.QuadUVS2(v0, v1, v2, v3, position, uvSettings);
